 ...
 * arbitrary conditions. The two most common conditions are that a class implements/extends another class, or that is it annotated with a specific
 * annotation. However, through the use of the {@link Test} class it is possible to search using arbitrary conditions.</p>
 *
 * <p>A ClassLoader is used to locate all locations (directories and jar files) in the class
 * path that contain classes within certain packages, and then to load those classes and check them. By default the ClassLoader returned by {@code
 * Thread.currentThread().getContextClassLoader()} is used, but this can be overridden by calling {@link #setClassLoader(ClassLoader)} prior to
 ...
 * a package name and a Test instance. This will cause the named package <b>and all sub-packages</b> to be scanned for classes that meet the test.
 * There are also utility methods for the common use cases of scanning multiple packages for extensions of particular classes, or classes annotated
 * with a specific annotation.</p>
 *
...
public class ResolverUtil<T> {
  ...
  public static class IsA implements Test {
    ...
    /**
     * 注册parent的class对象 Constructs an IsA test using the supplied Class as the parent class/interface.
     *
    ...
    public boolean matches(Class<?> type) {
      // 这个type不是null并且parent是这个class对象的超类
  ...
  /**
   * The ClassLoader to use when looking for classes. If null then the ClassLoader returned by Thread.currentThread().getContextClassLoader() will be
   * used.
   */
   ...
   * implementations. In the case of a non-interface class, subclasses will be collected.  Accumulated classes can be accessed by calling {@link
   * #getClasses()}.
   *
  ...
  /**
   * Scans for classes starting at the package provided and descending into subpackages. Each class is offered up to the Test as it is discovered, and
   * if the Test returns true the class is retained.  Accumulated classes can be fetched by calling {@link #getClasses()}.
   *
