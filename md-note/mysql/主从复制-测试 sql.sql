...
SELECT #NAME,
dept,MAX(age)maxage FROM mytbl2
GROUP BY dept
) ab 
...
select * from `t_emp` a inner join `t_dept` d on a.deptId = d.id

SELECT * FROM `t_emp` a left JOIN `t_dept` d ON a.deptId = d.id
  
SELECT * FROM `t_emp` a LEFT JOIN `t_dept` d ON a.deptId = d.id where d.id is NULL

SELECT * FROM `t_dept` d LEFT JOIN `t_emp` a ON a.deptId = d.id WHERE a.id is null

SELECT a.*, d.* FROM `t_emp` a LEFT JOIN `t_dept` d ON a.deptId = d.id 
union  #union 用于合并两个或多个查询语句的结果集，并消去表中任何重复行。查询语句要求 列的数量相同，列的数据类型相似，列的顺序相同
SELECT a.*, d.* FROM `t_dept` d LEFT JOIN `t_emp` a ON a.deptId = d.id WHERE a.id IS NULL


SELECT a.*, d.* FROM `t_emp` a LEFT JOIN `t_dept` d ON a.deptId = d.id 
UNION ALL #union 用于合并两个或多个查询语句的结果集，并消去表中任何重复行。查询语句要求 列的数量相同，列的数据类型相似，列的顺序相同
SELECT a.*, d.* FROM `t_dept` d LEFT JOIN `t_emp` a ON a.deptId = d.id WHERE a.id IS NULL
...
select c.name, ab.name from `t_emp` c left join 
...
EXPLAIN SELECT c.name, ab.name FROM `t_emp` c LEFT JOIN 
(SELECT b.id, a.name FROM `t_emp` a INNER JOIN `t_dept` b ON a.id = b.ceo)ab
ON c.deptId = ab.id
...
BEGIN    #开始  DECLARE声明一个变量
 DECLARE chars_str VARCHAR(100) DEFAULT 'abcdefghijklmnopqrstuvwxyzABCDEFJHIJKLMNOPQRSTUVWXYZ';
 DECLARE return_str VARCHAR(255) DEFAULT '';
 DECLARE i INT DEFAULT 0;
 WHILE i < n DO                              #RAND()*52  从0-51随机获取一个值  FLOOR()取整
